...
const SearchPage = () => {
  ...
  useEffect(() => {
    const fetchFlagData = async () => {
      try {
        const res = await fetch("https://restcountries.com/v3.1/all?fields=name,capital,flags,population,region,subregion,timezones,area");
  ...
  return (
    <Container style={{ marginTop: '100px' }}>
      {Array.isArray(filteredFlagData) && filteredFlagData.length > 0 ? (
